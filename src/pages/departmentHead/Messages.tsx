...
import { Plus, Send, Inbox, Mail, MessageSquare, Trash2, User } from 'lucide-react';
...
export default function Messages() {
  ...
  const handleViewConversation = async (conversation: Conversation) => {
    try {
      setSelectedUser(conversation.other_user_id);
      const response = await getConversation(conversation.other_user_id);
      if (response.success) {
        setSelectedConversation(response.messages);
      }
    } catch (error: any) {
  ...
  return (
    <>
      ...
      <div className="bg-white border-b border-slate-200 p-6 lg:p-8">
        ...
        <p className="text-slate-600 mt-2">Communicate with instructors and students in your department</p>
      </div>
            ...
            <form onSubmit={handleSendMessage} className="space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  {composeForm.message_text.length} / 5000 characters
                </p>
              </div>
